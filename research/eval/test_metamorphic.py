...
class TransformTests(unittest.TestCase):
    def test_mapping_and_unchanged_inputs(self):
        ...
        baseline, order, neutral = m.make_variants(CASE, random.Random(13))
        self.assertEqual(CASE, original)
        self.assertEqual(baseline["case"], CASE)
        self.assertNotEqual(order["canonical_indices"], [0, 1, 2])
        for variant in (baseline, order, neutral):
        ...
        self.assertEqual(list(neutral["case"][1]["intent"]["criteria"]), ["A", "B", "C"])
        neutral["case"][0]["utterance"] = "changed"
        self.assertEqual(CASE, original)
    ...
    def test_explicit_api_mapping_and_gold(self):
        ...
        neutral = m.neutralize_labels(case)
        self.assertEqual(permuted, m.permute_options(case, seed=42))
        self.assertEqual(neutral.canonical_to_transformed,
                         {"billing": "A", "technical": "B", "sales": "C"})
        self.assertEqual(neutral.transformed_to_canonical["A"], "billing")
        self.assertEqual(permuted.case.option_keys[permuted.case.gold_index], "billing")
        result = m.evaluate_variants(None, case, [permuted, neutral], score=semantic_scorer)
        ...
        self.assertEqual(result.variants[1]["canonical_to_transformed"], neutral.canonical_to_transformed)

    def test_many_options_and_nested_descriptions(self):
        case = deepcopy(CASE)
        case[1]["intent"]["criteria"] = {str(i): {"description": [str(i)]} for i in range(28)}
        neutral = m.make_variants(case, random.Random(0))[2]
        criteria = neutral["case"][1]["intent"]["criteria"]
        self.assertEqual(list(criteria)[-3:], ["Z", "AA", "AB"])
        criteria["A"]["description"].append("changed")
        self.assertEqual(case[1]["intent"]["criteria"]["0"], {"description": ["0"]})
...
class EvaluationTests(unittest.TestCase):
    ...
    def test_position_and_label_bias_detected_separately(self):
        ...
        self.assertEqual(report["neutral_label"]["semantic_agreement_rate"], 1)
        self.assertEqual(report["quality"]["baseline"], {"n_labelled": 0})
        def label_bias(cases):
            return [[0, 1, 0] if "A" in q["intent"]["criteria"] else probs
                    for (_, q), probs in zip(cases, semantic_scorer(cases))]
        report = m.evaluate([CASE], label_bias)["report"]
        self.assertEqual(report["option_order"]["semantic_agreement_rate"], 1)
        self.assertEqual(report["neutral_label"]["semantic_agreement_rate"], 0)
    ...
    def test_rendering(self):
        ...
        neutral = m.make_variants(CASE, random.Random(0))[2]["case"][1]["intent"]
        self.assertEqual(render_options({"t": "choice", "crit": neutral["criteria"]}),
                         ["A: payment issues", "B: software help", "C: new purchases"])
